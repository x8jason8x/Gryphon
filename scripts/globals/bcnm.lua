...
function checkNonTradeBCNM(player, npc, mode)
    ...
    local checks =
    {
        ...
        [170] = {
                    ...
                    [227] = function() return (player:getCurrentMission(ASA) == BATTARU_ROYALE and player:hasKeyItem(dsp.ki.DARK_SAP_CRYSTAL)) end, -- Battaru Royale
                },
        [179] = {
...
function CutsceneSkip(player, npc)
    ...
    elseif (Zone == 170) then -- Full Moon Fountain
        if ((player:hasCompletedQuest(WINDURST, THE_MOONLIT_PATH)) or (player:hasKeyItem(dsp.ki.WHISPER_OF_THE_MOON))) then -- The Moonlit Path
            skip = 1
        elseif ((player:hasCompletedMission(WINDURST, MOON_READING)) or (player:getCurrentMission(WINDURST) == MOON_READING and player:getVar("MissionStatus") == 2)) then -- Windy 9-2
            skip = 1
        end
