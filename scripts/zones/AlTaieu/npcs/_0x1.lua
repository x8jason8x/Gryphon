...
function onTrigger(player,npc)
    ...
    if (player:getCurrentMission(COP) == GARDEN_OF_ANTIQUITY and player:getVar("PromathiaStatus") == 2 and
        player:getVar("[SEA][AlTieu]SouthTower") == 0 and player:getVar("[SEA][AlTieu]SouthTowerCS") == 0 and not
        GetMobByID(AERNS_TOWER_SOUTH+0):isSpawned() and not GetMobByID(AERNS_TOWER_SOUTH+1):isSpawned() and not
        GetMobByID(AERNS_TOWER_SOUTH+2):isSpawned()) then
    ...
    elseif (player:getCurrentMission(COP) == GARDEN_OF_ANTIQUITY and player:getVar("PromathiaStatus") == 2 and
        player:getVar("[SEA][AlTieu]SouthTower") == 1 and player:getVar("[SEA][AlTieu]SouthTowerCS") == 0) then
