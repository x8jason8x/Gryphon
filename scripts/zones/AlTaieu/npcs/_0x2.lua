...
function onTrigger(player,npc)
    ...
    if (player:getCurrentMission(COP) == GARDEN_OF_ANTIQUITY and player:getVar("PromathiaStatus") == 2 and
        player:getVar("[SEA][AlTieu]WestTower") == 0 and player:getVar("[SEA][AlTieu]WestTowerCS") == 0 and not
        GetMobByID(AERNS_TOWER_WEST+0):isSpawned() and not GetMobByID(AERNS_TOWER_WEST+1):isSpawned() and not
        GetMobByID(AERNS_TOWER_WEST+2):isSpawned()) then
    ...
    elseif (player:getCurrentMission(COP) == GARDEN_OF_ANTIQUITY and player:getVar("PromathiaStatus") == 2 and
        player:getVar("[SEA][AlTieu]WestTower") == 1 and player:getVar("[SEA][AlTieu]WestTowerCS") == 0) then
